...
object DenGL {
    ...
    const val WATER_FS = """
        precision mediump float;
        uniform vec3 uCol;
        uniform float uFlat;   // 0 = a glowy pond, 1 = a solid biome sea
        varying vec2 vUv; varying float vH;
        void main() {
            float r = length(vUv);
            float edge = 1.0 - smoothstep(0.72, 1.0, r);
            // Ponds glow softly; seas stay opaque out to the horizon rim.
            float a = mix(edge * 0.6, edge * 0.35 + 0.6, uFlat);
