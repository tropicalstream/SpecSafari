...
object Sprites {
    ...
    fun creature(c: Canvas, species: Int, x: Float, y0: Float, s: Float, t: Float, excited: Boolean) {
        ...
        when (species) {
            ...
            26 -> { // MOLDEWARP — the velvet digger, mid-emergence
                ...
                for (i in 0 until 22) {
                    val a = (i / 22f) * 6.283f
                    stroke.color = sp.accent; stroke.strokeWidth = s * 0.045f
            ...
            27 -> { // SYLVARCH — glade archivist with sensory crown and fan tail
                body(c, sp, x, y, s * 0.92f)
                fill.color = sp.main
                c.drawOval(RectF(x - s * 0.58f, y + s * 0.32f, x - s * 0.35f, y + s * 1.25f), fill)
                c.drawOval(RectF(x + s * 0.35f, y + s * 0.32f, x + s * 0.58f, y + s * 1.25f), fill)
                // Cache-working forepaws.
                fill.color = sp.accent
                c.drawOval(RectF(x - s * 0.82f, y + s * 0.55f, x - s * 0.30f, y + s * 0.82f), fill)
                c.drawOval(RectF(x + s * 0.30f, y + s * 0.55f, x + s * 0.82f, y + s * 0.82f), fill)
                // Fan tail, three leaf-like vanes.
                for (k in -1..1) {
                    path.reset(); path.moveTo(x + k * s * 0.18f, y + s * 0.35f)
                    path.quadTo(x + k * s * 0.62f, y + s * 0.72f,
                        x + k * s * 0.45f, y + s * 1.36f)
                    path.quadTo(x + k * s * 0.05f, y + s * 1.05f, x, y + s * 0.48f)
                    path.close(); fill.color = if (k == 0) sp.main else sp.accent; c.drawPath(path, fill)
                }
                // Polarization crown branches.
                stroke.color = sp.accent; stroke.strokeWidth = s * 0.10f
                for (sx in intArrayOf(-1, 1)) {
                    c.drawLine(x + sx * s * 0.16f, y - s * 0.55f,
                        x + sx * s * 0.38f, y - s * 1.28f, stroke)
                    c.drawLine(x + sx * s * 0.30f, y - s * 0.98f,
                        x + sx * s * 0.68f, y - s * 1.12f, stroke)
                    c.drawLine(x + sx * s * 0.32f, y - s * 1.02f,
                        x + sx * s * 0.14f, y - s * 1.42f, stroke)
                }
                eyes(c, x, y - s * 0.18f, s * 0.86f)
            }
            28 -> { // MISTCROWN — moss-mantled six-limbed canopy reasoner
                // Balancing tail behind the body.
                stroke.color = sp.main; stroke.strokeWidth = s * 0.30f
                path.reset(); path.moveTo(x, y + s * 0.45f)
                path.cubicTo(x + s * 1.15f, y + s * 0.65f,
                    x + s * 1.0f, y - s * 0.35f, x + s * 0.56f, y - s * 0.52f)
                c.drawPath(path, stroke)
                body(c, sp, x, y, s * 0.88f)
                // Three prehensile limb pairs.
                stroke.color = sp.accent; stroke.strokeWidth = s * 0.16f
                for (pair in 0..2) for (sx in intArrayOf(-1, 1)) {
                    val yy = y + s * (0.15f + pair * 0.32f)
                    c.drawLine(x + sx * s * 0.34f, yy,
                        x + sx * s * (0.82f + pair * 0.10f), yy + s * 0.20f, stroke)
                    fill.color = sp.accent
                    c.drawCircle(x + sx * s * (0.86f + pair * 0.10f), yy + s * 0.22f, s * 0.12f, fill)
                }
                // Moss chromatophore mantle.
                fill.color = sp.accent
                for (k in 0..5) c.drawCircle(x - s * 0.48f + k * s * 0.19f,
                    y + s * 0.30f + (k % 2) * s * 0.13f, s * 0.18f, fill)
                // Seven chemical-sensing crown tendrils.
                stroke.strokeWidth = s * 0.07f
                for (k in 0 until 7) {
                    val a = (k / 7f) * 6.283f
                    val ex = x + cos(a) * s * 0.56f
                    val ey = y - s * 0.66f + sin(a) * s * 0.28f
                    c.drawLine(x, y - s * 0.36f, ex, ey, stroke)
                    c.drawCircle(ex, ey, s * 0.07f, fill)
                }
                eyes(c, x, y - s * 0.13f, s * 0.82f)
            }
            else -> {
                body(c, sp, x, y, s)
                eyes(c, x, y - s * .1f, s * .8f)
            }
        }
    }
